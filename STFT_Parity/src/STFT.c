...
//
// Production model-pilot pipeline:
//   1. Receive signed first differences prepared from 257 captured samples
//   2. Left-shift by DIFF_SHIFT and saturate into Q15
...
#define STFT_PARITY_MODE 1
#include "STFT.h"
#include "window_q15.h"     // Q15 Hanning coefficients
#include <DSPLib.h>

#ifdef STFT_PARITY_MODE
...
void STFT_compute_next_segment(int16_t *stft_input_I, int16_t *stft_input_Q)
{
...
#ifdef STFT_PARITY_MODE
    /* Acquisition is stopped in parity mode, so this blocking diagnostic
     * transmission cannot perturb sample timing. It exposes the exact Q15
     * window-multiply result used as the FFT input. */
    Parity_sendQ15Stage(PARITY_STAGE_WINDOWED, cmplx_buf, 2 * FFT_SIZE);
#endif
...
#ifdef STFT_PARITY_MODE
    /* Preserve the exact LEA/DSPLib FFT output for stage-by-stage comparison
     * before the CPU magnitude/log2 loop converts it into a column. */
    Parity_sendQ15Stage(PARITY_STAGE_FFT, cmplx_buf, 2 * FFT_SIZE);
#endif
    ...
    for (n = 0; n < FFT_SIZE; n++)
    {
        ...
        // in practice. The sum is uint32_t so the (-32768,-32768) corner
        // remains well-defined instead of overflowing signed int32_t.
