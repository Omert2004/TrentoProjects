"""Integrity-protected protocol shared by AI_Phase firmware and host tools.
...
Frame = Union[ColumnPacket, RatePacket, ProfilePacket]
...
class StftFrameReader:
    """Persistent, overlap-safe reader for multiplexed D0/D2/D3 frames."""
    ...
    def _track_column(self, packet: ColumnPacket) -> None:
        if self._last_column_sequence is not None:
            ...
            if packet.column_sequence != expected:
                delta, forward = self._forward_delta(packet.column_sequence, expected, 1 << 16)
                if forward:
    ...
    def _track_report_sequence(self, current: int, *, kind: str) -> None:
        ...
        if previous is not None:
            ...
            if current != expected:
                ...
                counter = f"{kind}_sequence_gaps" if forward else f"{kind}_sequence_reorders"
        ...
        while True:
            ...
            if marker == COLUMN_MARKER:
                ...
                if any(value < COLUMN_MIN or value > COLUMN_MAX for value in values):
                    ...
                    continue
                frame: Frame = ColumnPacket(sequence, first_index, drops, values, now, candidate)
            elif marker == RATE_MARKER:
                report_sequence, accepted, drops = struct.unpack_from("<HHI", candidate, 3)
                frame = RatePacket(report_sequence, accepted, drops, now, candidate)
            else:
                report_sequence, hops, stft_ticks, dma_ticks = struct.unpack_from("<HHHH", candidate, 3)
                frame = ProfilePacket(report_sequence, hops, stft_ticks, dma_ticks, now, candidate)
...
def encode_column(
    *,
    ...
    body = bytes([COLUMN_MARKER]) + struct.pack(
        "<HII", column_sequence & 0xFFFF, first_new_sample_index & 0xFFFFFFFF,
        cumulative_drop_count & 0xFFFFFFFF,
    ) + bytes(values)
    return _with_crc(body)


def encode_rate(*, report_sequence: int, accepted_samples: int, cumulative_drop_count: int) -> bytes:
    return _with_crc(bytes([RATE_MARKER]) + struct.pack(
        "<HHI", report_sequence & 0xFFFF, accepted_samples & 0xFFFF,
        cumulative_drop_count & 0xFFFFFFFF
    ))


def encode_profile(*, report_sequence: int, hop_count: int, stft_ticks: int, dma_wait_ticks: int) -> bytes:
    return _with_crc(bytes([PROFILE_MARKER]) + struct.pack(
        "<HHHH", report_sequence & 0xFFFF, hop_count & 0xFFFF,
        stft_ticks & 0xFFFF, dma_wait_ticks & 0xFFFF
    ))
