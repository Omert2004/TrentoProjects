...
"""Export balanced near/mid embedded-Q15 tensors for sessions 01 through 10.
...
The split deliberately includes both the older acquisition domain (sessions
01-05) and the newer acquisition domain (sessions 06-10) in train, validation,
and test. Far captures from the older sessions are ignored so every session
uses the same near/mid-only capture matrix.
"""
...
def main() -> int:
    ...
    try:
        for metadata_path in sorted(input_root.rglob("*.metadata.json")):
            ...
            if gesture_class == "empty":
                if distance != "na":
                    raise ValueError(f"empty capture has a distance: {metadata_path}")
            elif distance not in DISTANCES:
                if distance == "far":
                    ignored_excluded_distance += 1
                    continue
                raise ValueError(f"invalid gesture distance {distance!r}: {metadata_path}")
        ...
        expected_counts = {
            ("train", gesture_class): 180 for gesture_class in PILOT_CLASSES
        } | {
            ("valid", gesture_class): 60 for gesture_class in PILOT_CLASSES
        } | {
            ("test", gesture_class): 60 for gesture_class in PILOT_CLASSES
        }
        ...
        summary = {
            ...
            "included_gesture_distances": list(DISTANCES),
            "filename_format": (
            ...
            "ignored_excluded_distance_captures": ignored_excluded_distance,
            "counts": [
    ...
    print(f"Ignored far captures: {ignored_excluded_distance}")
    print(f"Exported tensors:  {len(entries)} (300 per class)")
