...
```bash
python frame_timing_check.py --port COM7 --baud 115200 --sampling-rate 2000 --duration 30
```

Capture a fixed-duration file:

```bash
python raw_serial_capture.py --port COM7 --baud 115200 \
...
```bash
python raw_capture.py --port COM7 --sampling-rate 2000 --default-duration 2
```
...
```bash
python visualizer.py captures/example.csv
```
