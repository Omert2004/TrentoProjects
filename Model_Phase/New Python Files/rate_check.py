...
from stft_protocol import (
    ColumnPacket,
    StftFrameReader,
    serial_capacity_columns_per_second,
)
...
def main() -> int:
    ...
    print(
        "UART column-only ceiling: "
        f"{serial_capacity_columns_per_second(args.baud):.2f} columns/s"
    )
    if intervals:
        ordered = sorted(intervals)
        p95 = ordered[min(len(ordered) - 1, int(0.95 * len(ordered)))]
        print(
            f"Host inter-arrival: mean={statistics.fmean(intervals):.2f} ms, "
            f"p95={p95:.2f} ms, max={max(intervals):.2f} ms"
        )
