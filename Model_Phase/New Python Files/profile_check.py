...
def main() -> int:
    ...
    try:
        while time.perf_counter() - start < args.duration:
            ...
            if isinstance(frame, RatePacket):
                ...
                print(
                    f"[{t:6.2f}s] RATE report={frame.report_sequence:5d}, "
                    f"accepted={frame.accepted_samples:5d} ({pct:5.1f}%), "
                ...
                print(
                    f"[{t:6.2f}s] PROFILE report={frame.report_sequence:5d}, "
                    f"hops={frame.hop_count:3d}; "
                    f"STFT={stft_ms:7.1f} ms ({stft_ms / divisor:6.2f}/hop); "
                    f"DMA wait={dma_ms:7.1f} ms "
                    f"({dma_ms / divisor:6.2f}/hop)"
                )
    ...
    passed = (
        ...
        and stats.first_reported_drop_count in (None, 0)
    )
    print(
        f"\nProfile reports: {profiles}; "
        f"validated columns skipped: {stats.columns_accepted}"
    )
