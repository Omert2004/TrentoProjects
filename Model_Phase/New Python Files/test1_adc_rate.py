...
def main() -> int:
    ...
    if args.baud <= 0 or args.sampling_rate <= 0 or args.duration <= 0:
        print(
            "Error: baud, sampling rate, and duration must be positive.",
            file=sys.stderr,
        )
        return 2
        ...
        while time.perf_counter() - start < args.duration:
            ...
            if isinstance(frame, RatePacket):
                ...
                print(
                    ...
                    f"report={frame.report_sequence:5d}, "
                    f"accepted={frame.accepted_samples:5d} samples/s "
                    f"({pct:5.1f}%), MCU drops={frame.cumulative_drop_count}"
                )
