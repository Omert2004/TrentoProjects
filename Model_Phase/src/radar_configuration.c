/* Peripheral setup and CRC-protected D0/D2/D3/D4 UART framing. */
...
void UART_putCnnResultFrame(uint16_t inference_sequence,
                            uint16_t last_column_sequence,
                            uint8_t predicted_class,
                            const int32_t logits[4])
{
    uint8_t frame[26];
    uint16_t offset = 0;
    uint16_t crc;
    ...
    frame[offset++] = 0xAA;
    frame[offset++] = 0x55;
    frame[offset++] = CNN_RESULT_MARKER;
    put_u16_le(frame, &offset, inference_sequence);
    put_u16_le(frame, &offset, last_column_sequence);
    frame[offset++] = predicted_class;
    for (i = 0; i < 4; i++)
        put_u32_le(frame, &offset, (uint32_t)logits[i]);
    crc = crc16_ccitt(&frame[2], (uint16_t)(offset - 2));
    put_u16_le(frame, &offset, crc);
    put_blocking_frame(frame, offset);
}
...
void Init_ADC(void)
{
    ...
    // (ADC12ENC | ADC12SC) on every 2 kHz tick to compensate. Setting
...
{
    // TA2 up-mode timer at SAMPLING_RATE_HZ (2 kHz). CCR1's output feeds
    ...
    // pulse per period. The compare event is placed near the midpoint.
